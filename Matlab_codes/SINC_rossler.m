function results = SINC_rossler(x,x_dot,r,epsilon,Gamma)
    ...
    for network_iteration = 1:Gamma
        ...
        for i = 1:N
            ...
            F = Estimate_F_matrix(x(:,3*(i-1)+1:3*i),r);
            Z = Estimate_Z_matrix(x(:,1:3:end),r,i);
            ...
            for iter = 1:100
                ...
                B_hat = B_hat*norm(Lambda_hat_updated);
                Lambda_hat_updated = Lambda_hat_updated/norm(Lambda_hat_updated);
                ...
                if(norm(Lambda_hat_updated-Lambda_hat)<epsilon || iter == 100)
                    ...
                    %B_each_node((network_iteration-1)*N+i,:) = B_hat';
                    %A_each_node((network_iteration-1)*N+i,:) = A_hat';
                    break;
                end
                Lambda_hat = Lambda_hat_updated;
            end
    ...
    Lambda = mean(Lambda_each_node((Gamma-1)*N:Gamma*N,:));
    for i = 1:N
        F = Estimate_F_matrix(x(:,3*(i-1)+1:3*i),r);
        Z = Estimate_Z_matrix(x(:,1:3:end),r,i);
...
function Z = Estimate_Z_matrix(x,r,node_index)
    ...
    for t = 1:M
        ...
        for j=0:r-1
            for k = 1:r
                Z_i(:,(j)*r+k) = (x(t,node_index)^j).*(x_temp(t,:)'.^k);
            end
        end
